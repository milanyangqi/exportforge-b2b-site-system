...
function mergeOptionalLocalizedText(existing: Partial<Record<LocaleCode, string>> | undefined, localeCode: LocaleCode, value: string): Translation | undefined {
  const trimmedValue = value.trim();
  const next = { ...(existing ?? {}) } as Partial<Record<LocaleCode, string>>;

  if (trimmedValue) {
    next[localeCode] = trimmedValue;
    if (!next.en) next.en = trimmedValue;
    if (!next.zh) next.zh = trimmedValue;
  } else {
    delete next[localeCode];
  }

  const fallbackValue = Object.values(next).find((item) => item?.trim())?.trim();
  if (!fallbackValue) return undefined;

  return {
    ...next,
    en: next.en?.trim() ? next.en : fallbackValue
  };
}
...
export function AdminApp({ email, initialTab, locale }: { email: string; initialTab?: string; locale: LocaleCode }) {
  ...
  function submitProductForm() {
    ...
    const seoTitle = productForm.seoTitleZh || productForm.seoTitleEn;
    const seoDescription = productForm.seoDescriptionZh || productForm.seoDescriptionEn;
    const nextProduct: ProductCategory = {
      ...
      seo: {
        ...(existingProduct?.seo ?? {}),
        title: mergeOptionalLocalizedText(existingProduct?.seo?.title, locale, seoTitle),
        description: mergeOptionalLocalizedText(existingProduct?.seo?.description, locale, seoDescription),
  ...
  const navigationProductOptions = [...state.products].sort((a, b) => (
    (pickLocalizedText(a.name, locale) || a.slug).localeCompare(pickLocalizedText(b.name, locale) || b.slug)
  ));
  ...
  const visualBuilderModules = [
    ...
    ...templateSettings.customBlocks.map((block) => ({
      ...
      label: pickLocalizedText(block.title, locale),
    ...
    if (selectedCustomBlock) {
      return (
        <div className="visual-properties-panel">
          ...
          <label>眉标
            <input value={pickLocalizedText(selectedCustomBlock.eyebrow, locale)} onChange={(event) => updateCustomTemplateBlock(selectedCustomBlock.id, { eyebrow: mergeLocalizedText(selectedCustomBlock.eyebrow, locale, event.target.value, event.target.value) })} />
          </label>
          <label>标题
            <input value={pickLocalizedText(selectedCustomBlock.title, locale)} onChange={(event) => updateCustomTemplateBlock(selectedCustomBlock.id, { title: mergeLocalizedText(selectedCustomBlock.title, locale, event.target.value, event.target.value) })} />
          </label>
          <label>正文
            <textarea value={pickLocalizedText(selectedCustomBlock.body, locale)} onChange={(event) => updateCustomTemplateBlock(selectedCustomBlock.id, { body: mergeLocalizedText(selectedCustomBlock.body, locale, event.target.value, event.target.value) })} />
          </label>
                ...
                {(selectedCustomBlock.imageItems ?? getCustomBlockImages(selectedCustomBlock)).map((item) => (
                  <article className={item.enabled ? "visual-image-item" : "visual-image-item disabled"} key={item.id}>
                    ...
                    <div className="visual-image-item-fields">
                      ...
                      <input disabled={!canManageFrontendSettings} placeholder="图片说明" value={pickLocalizedText(item.caption, locale)} onChange={(event) => updateCustomImageItem(selectedCustomBlock, item.id, { caption: mergeLocalizedText(item.caption, locale, event.target.value, event.target.value) })} />
                    </div>
          ...
          {selectedCustomBlock.type === "cta" ? (
            <>
              <label>按钮文字
                <input value={pickLocalizedText(selectedCustomBlock.buttonLabel, locale) || "发送询盘"} onChange={(event) => updateCustomTemplateBlock(selectedCustomBlock.id, { buttonLabel: mergeLocalizedText(selectedCustomBlock.buttonLabel, locale, event.target.value, event.target.value) })} />
              </label>
              <label>按钮链接
	            ...
	            <div className="visual-front-navigation-inner">
	              <div className="visual-front-brand">
	                ...
	                {renderVisualTextTarget({
	                  ...
	                  value: state.siteSettings.title || "Export site",
	                  element: "strong",
          ...
          <div className="visual-custom-module-body">
            ...
            <div className="visual-custom-copy">
              {renderVisualTextTarget({
                ...
                element: "span",
                className: "eyebrow",
                onCommit: (value) => updateCustomTemplateBlock(block.id, { eyebrow: mergeLocalizedText(block.eyebrow, locale, value, value) })
              })}
              {renderVisualTextTarget({
                ...
                element: "h3",
                multiline: true,
                onCommit: (value) => updateCustomTemplateBlock(block.id, { title: mergeLocalizedText(block.title, locale, value, value) })
              })}
              {renderVisualTextTarget({
                ...
                multiline: true,
                allowEmpty: true,
                onCommit: (value) => updateCustomTemplateBlock(block.id, { body: mergeLocalizedText(block.body, locale, value, value) })
              })}
              {block.type === "cta" ? (
          ...
          return (
            ...
            >
              <div className="nav-hierarchy-cell">
                ...
                <label>导航名称
                  <input disabled={!canManageFrontendSettings} value={pickLocalizedText(item.label, locale)} onChange={(event) => updateNavigationItem(item.id, { label: mergeLocalizedText(item.label, locale, event.target.value, event.target.value) })} />
                </label>
                  ...
                  {parentOptions.map((parent) => {
                    ...
                    return (
                      <option key={parent.id} value={parent.id}>
                        {`${parentDepth + 1}级 · ${pickLocalizedText(parent.label, locale)}`}
                      </option>
                    );
                  })}
                </select>
                ...
                {state.contactChannels.map((channel, index) => {
                  ...
                  <article className="admin-edit-card compact contact-card" key={channel.id}>
                    ...
                    <div className="contact-icon-manager">
                      {channel.iconUrl ? (
                        <Image className="contact-icon-thumb" src={channel.iconUrl} alt={`${channelLabel} 图标`} width={34} height={34} unoptimized />
                      ) : (
