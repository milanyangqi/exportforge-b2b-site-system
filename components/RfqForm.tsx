...
const labels = {
  ...
  workpieceMaterial: { en: "Material / fibre", zh: "材料 / 纱线", ar: "مادة الشغل" },
  ...
  success: { en: "Your inquiry has been submitted.", zh: "询盘已提交。", ar: "تم إرسال الطلب. سيراجع فريق Rivermake التفاصيل ويتابع معك قريبًا." },
  ...
  return (
    <form className="rfq-form" onSubmit={submit}>
      ...
      <label>
        ...
        <input name="productType" required placeholder="Crochet throw, woven basket, knit cushion" />
      </label>
      <label>
      ...
      <label>
        ...
        <input name="destination" placeholder="United States, United Kingdom, France..." />
      </label>
      <label>
        {text("workpieceMaterial", locale)}
        <input name="workpieceMaterial" placeholder="Cotton, wool, recycled yarn..." />
      </label>
      <label className="wide">
        {text("message", locale)}
        <textarea name="message" placeholder="Tell us about pieces, colours, materials, quantity and destination..." />
      </label>
      {status === "success" ? (
