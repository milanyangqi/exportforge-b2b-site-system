...
function buildImagePrompt(payload: ImagePayload) {
  const title = payload.title?.trim() || "B2B bamboo skewers article";
  const excerpt = payload.excerpt?.trim() || "";
  const body = stripMarkdown(payload.body ?? "").slice(0, 1800);

  return [
    "Create a professional square hero image for a B2B sourcing article on a bamboo-skewer website named GrillBeats.",
    "Style: realistic industrial product photography, soft daylight, natural bamboo skewers and paper packaging, export-ready, no text, no logos, no watermarks.",
    `Article title: ${title}`,
    excerpt ? `Article excerpt: ${excerpt}` : "",
    body ? `Article body context: ${body}` : "",
    "Avoid people, charts, UI screenshots, written words, distorted skewers, unsafe food handling, and dark blurry backgrounds."
  ].filter(Boolean).join("\n");
}
