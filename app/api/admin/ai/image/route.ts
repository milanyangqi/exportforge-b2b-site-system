...
function buildImagePrompt(payload: ImagePayload) {
  const title = payload.title?.trim() || "B2B home fragrance article";
  const excerpt = payload.excerpt?.trim() || "";
  const body = stripMarkdown(payload.body ?? "").slice(0, 1800);

  return [
    "Create a professional square hero image for a B2B home fragrance article on a website named CedarOrigin.",
    "Style: realistic botanical product photography, warm Nordic daylight, reed diffusers, scented candles and room sprays, no text, no logos, no watermarks.",
    `Article title: ${title}`,
