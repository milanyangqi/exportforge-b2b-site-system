...
function buildImagePrompt(payload: ImagePayload) {
  const title = payload.title?.trim() || "B2B 3D printing article";
  const excerpt = payload.excerpt?.trim() || "";
  const body = stripMarkdown(payload.body ?? "").slice(0, 1800);

  return [
    "Create a professional square hero image for a B2B technical article on a custom 3D printing website named CuriousMake.",
    "Style: realistic industrial product photography, clean product studio lighting, 3D printed functional parts and material samples, export-ready, no text, no logos, no watermarks.",
    `Article title: ${title}`,
