import { articles, pages, templateDefaults, contactChannels, defaultEnabledLocales, defaultNavigation, productCategories, siteSettings, uploadedFiles } from "@/data/site";
...
const currentTemplateContentVersion = "current-template-grillbeats-wholesale-v1";
...
const defaultHeroSlides: SiteHeroSlide[] = templateDefaults.heroSlides;

const defaultSiteSettings: SiteSettings = {
  title: siteSettings.brand,
  tagline: "Bamboo skewers, wholesale enquiries and custom packaging.",
  ...
  mailReplyTemplate: "Hello {name},\n\nThank you for your RFQ about {productType}. We have received your inquiry and will follow up with product details, quotation, and lead time soon.\n\nBest regards,\n{siteTitle}",
  ...
  privacySummary: "We use submitted RFQ details only for product quotation, sales follow-up, and service improvement."
};

const defaultTemplateTextBlocks: Record<string, Translation> = templateDefaults.textBlocks;
...
const defaultTemplateSettings: SiteTemplateSettings = templateDefaults;
...
export function createDefaultAdminState(): AdminState {
  ...
  const state: AdminState = {
    ...
    aiSettings: {
      ...
      defaultLocale: "en",
      brandVoice: "Clear, practical bamboo skewer copy for small wholesale buyers.",
      targetMarkets: ["Europe", "North America", "Southeast Asia", "MENA"],
      requiredKeywords: ["bamboo skewers", "wholesale", "custom packaging"],
      blockedWords: [],
...
function normalizeActiveTemplateContactChannels(channels?: AdminState["contactChannels"]) {
...
export async function readAdminState(): Promise<AdminState> {
  ...
  const raw = kv ? await kv.get(stateKey) : await readLocalStateFile().catch((error: NodeJS.ErrnoException) => {
