import { articles, contactChannels, defaultEnabledLocales, defaultNavigation, pages, productCategories, siteSettings, uploadedFiles } from "@/data/site";
...
const currentTemplateContentVersion = "cedarorigin-nordic-botanical-v1";
...
const defaultHeroSlides: SiteHeroSlide[] = [
  { id: "cedar-hero", imageUrl: "/assets/current-template/hero.jpg", alt: { en: "Botanical home fragrance collection", zh: "植物系家居香氛系列" }, enabled: true, order: 10 }
];

const defaultSiteSettings: SiteSettings = {
  title: siteSettings.brand,
  tagline: "Botanical home fragrance collections for brand buyers.",
  contentVersion: currentTemplateContentVersion,
  siteIconUrl: "",
  fontFamily: "\"DM Sans\", \"PingFang SC\", sans-serif",
  siteUrl: process.env.NEXT_PUBLIC_SITE_URL ?? "https://cedarorigin-029.437991663.workers.dev",
  ...
  mailReplyTemplate: "Hello {name},\n\nThank you for your inquiry about {productType}. We have received your brief and will follow up after reviewing the details.\n\nBest regards,\n{siteTitle}",
  ...
  dateFormat: "Y-m-d",
  timeFormat: "H:i",
  defaultArticleCategory: "home-fragrance",
  ...
  privacySummary: "We use submitted inquiry details to respond to your fragrance project request."
};

const defaultTemplateTextBlocks: Record<string, Translation> = {
  productsEyebrow: { en: "Collections", zh: "产品系列" },
  productsTitle: { en: "Fragrance for every kind of space.", zh: "让香气融入每一种空间。" },
  productsBody: { en: "Explore reed diffusers, scented candles and room sprays.", zh: "探索藤条香薰、香薰蜡烛和室内喷雾。" },
  factoryEyebrow: { en: "OEM / ODM", zh: "定制合作" },
  factoryTitle: { en: "Your idea, thoughtfully made.", zh: "从构想到完整系列。" },
  marketsEyebrow: { en: "Making & quality", zh: "制作与品质" },
  marketsTitle: { en: "Considered at every step.", zh: "认真对待每一步。" },
  articlesEyebrow: { en: "Ideas", zh: "灵感" },
  articlesTitle: { en: "Explore the world of home fragrance.", zh: "探索家居香氛。" },
  rfqEyebrow: { en: "Start an inquiry", zh: "提交询盘" },
  rfqTitle: { en: "Tell us what you imagine.", zh: "告诉我们您的香氛构想。" }
};
...
const defaultTemplateSettings: SiteTemplateSettings = {
  homeTemplate: "catalog-focus",
  heroKicker: { en: "BOTANICAL HOME FRAGRANCE", zh: "植物系家居香氛" },
  heroTitle: { en: "Nature. Fragrance. A brighter tomorrow.", zh: "以自然香气，点亮每个日常。" },
  heroBody: { en: "Discover considered reed diffusers, scented candles and room sprays for your next collection.", zh: "探索藤条香薰、香薰蜡烛与室内喷雾，为下一组品牌系列寻找灵感。" },
  primaryCtaLabel: { en: "Start an inquiry", zh: "提交询盘" },
  secondaryCtaLabel: { en: "Explore products", zh: "探索产品" },
  heroCarouselEnabled: false,
  heroCarouselAutoplay: false,
  heroCarouselIntervalSeconds: 7,
  heroSlides: defaultHeroSlides,
  showHeroVisual: true,
  showHeroMetrics: false,
  footerTagline: { en: "Botanical home fragrance collections for brand buyers.", zh: "为品牌买家呈现植物系家居香氛系列。" },
  footerCopyright: { en: "Copyright © {year} {brand}. All rights reserved.", zh: "Copyright © {year} {brand}. All rights reserved." },
  footerCredit: { en: "Home fragrance concepts for thoughtful brands.", zh: "用心设计的家居香氛方向。" },
  homeProductCount: 3,
  homeArticleCount: 0,
  visibleSections: { navigation: true, hero: true, products: true, factory: true, markets: true, articles: true, rfq: true },
  sectionOrder: { navigation: 10, hero: 20, products: 30, factory: 40, markets: 50, articles: 60, rfq: 70 },
...
export function createDefaultAdminState(): AdminState {
  ...
  const state: AdminState = {
    ...
    aiSettings: {
      ...
      defaultLocale: "en",
      brandVoice: "Calm, clear home fragrance copy for CedarOrigin brand buyers.",
      targetMarkets: ["Europe", "North America"],
      requiredKeywords: ["reed diffusers", "scented candles", "room sprays", "OEM / ODM"],
      blockedWords: [],
...
function normalizeActiveTemplateContactChannels(existingChannels = contactChannels) {
  return mergeContactChannels(existingChannels).map((channel) => {
    if (channel.id === "email" && (channel.value === "sales@example.com" || channel.href === "mailto:sales@example.com")) {
      return { ...channel, value: "", href: "mailto:" };
    }

    if (channel.id === "wechat" && channel.value === "ExportFactory") {
      return { ...channel, value: "CedarOrigin" };
    }

    if (channel.value === "ExportForge") {
      return { ...channel, value: "CedarOrigin", href: channel.href.replace(/exportforge/gi, "cedarorigin") };
    }

    if (channel.value.toLowerCase().includes("exportforge") || channel.href.toLowerCase().includes("exportforge")) {
      return {
        ...channel,
        value: channel.value.replace(/exportforge/gi, "cedarorigin"),
        href: channel.href.replace(/exportforge/gi, "cedarorigin")
      };
    }
