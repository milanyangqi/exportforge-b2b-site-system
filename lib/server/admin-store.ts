...
const currentTemplateContentVersion = "current-template-curiousmake-rfq-11-v1";
...
const defaultHeroSlides: SiteHeroSlide[] = [{
  ...
  alt: { en: "Illustrative custom 3D printed bracket", zh: "定制 3D 打印支架示意图" },
  enabled: true,
  order: 10
}];
...
const defaultSiteSettings: SiteSettings = {
  ...
  tagline: "Custom 3D printing inquiries based on your drawing and requirements.",
  contentVersion: currentTemplateContentVersion,
  siteIconUrl: "",
  fontFamily: "\"Manrope\", \"PingFang SC\", \"Microsoft YaHei\", sans-serif",
  siteUrl: process.env.NEXT_PUBLIC_SITE_URL ?? "https://curiousmake-030.437991663.workers.dev",
  ...
  mailReplyTemplate: "Hello {name},\n\nThank you for your custom 3D printing inquiry about {productType}. We will review the details you supplied and contact you about next steps.\n\nBest regards,\n{siteTitle}",
  ...
  privacySummary: "We use submitted RFQ details and drawings to review your request and contact you about a quotation."
};

const defaultTemplateTextBlocks: Record<string, Translation> = {
  productsTitle: { en: "Illustrative custom part forms", zh: "定制零件结构示意" },
  productsBody: { en: "Send your drawing for a feasibility review and quotation.", zh: "提交图纸以评估可制造性并获取报价。" },
  rfqTitle: { en: "Tell us what you need to make.", zh: "告诉我们您要加工什么。" },
  rfqBody: { en: "Upload a CAD file and share quantity, material, finish and delivery requirements.", zh: "上传 CAD 文件并说明数量、材料、表面及交付要求。" }
};
...
const defaultTemplateSettings: SiteTemplateSettings = {
  ...
  heroKicker: { en: "Custom 3D printing for global buyers", zh: "面向海外客户的定制 3D 打印" },
  heroTitle: { en: "Your CAD file. A real part. A clearer path forward.", zh: "从图纸出发，定制您需要的零件。" },
  heroBody: {
    en: "Send your 3D model and requirements to discuss a custom part. We review geometry, material preference, quantity and delivery needs before quoting.",
    zh: "提交 3D 模型与需求，沟通零件结构、材料偏好、数量和交付要求后再报价。"
  },
  primaryCtaLabel: { en: "Upload CAD & Request a Quote", zh: "上传图纸获取报价" },
  secondaryCtaLabel: { en: "Part Gallery", zh: "零件图库" },
  ...
  footerTagline: {
    en: "Custom 3D printing inquiries for overseas buyers.",
    zh: "面向海外客户的定制 3D 打印询价。"
  },
  footerCopyright: {
...
export function createDefaultAdminState(): AdminState {
  ...
  const state: AdminState = {
    ...
    aiSettings: {
      ...
      defaultLocale: "en",
      brandVoice: "Clear, evidence-based copy for custom 3D printing inquiries. Do not invent capabilities.",
      targetMarkets: [],
      requiredKeywords: ["custom 3D printing", "CAD file", "request a quote"],
      blockedWords: [],
...
function normalizeActiveTemplateContactChannels(existingChannels = contactChannels) {
  return mergeContactChannels(existingChannels).map((channel) => {
    if (channel.id === "email" && (channel.value === "sales@example.com" || channel.href === "mailto:sales@example.com")) {
      return { ...channel, value: "sales@curiousmake.com", href: "mailto:sales@curiousmake.com" };
    }

    if (channel.id === "wechat" && channel.value === "ExportFactory") {
      return { ...channel, value: "CuriousMake" };
    }

    if (channel.value === "ExportForge") {
      return { ...channel, value: "CuriousMake", href: channel.href.replace(/exportforge/gi, "curiousmake") };
    }

    if (channel.value.toLowerCase().includes("exportforge") || channel.href.toLowerCase().includes("exportforge")) {
      return {
        ...channel,
        value: channel.value.replace(/exportforge/gi, "curiousmake"),
        href: channel.href.replace(/exportforge/gi, "curiousmake")
      };
    }
